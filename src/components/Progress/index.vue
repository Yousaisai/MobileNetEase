...
export default {
  ...
  mounted() {
    this.btnWidth =
      document.getElementsByClassName("progress-btn")[0].clientWidth;
  },
  methods: {
    // 点击按钮
    ...
    _triggerPercent() {
      ...
      const percent1 = Math.min(1, this.$refs.progress.clientWidth / barWidth);
      this.$emit("percentChange", percent1);
    },
    // 设置偏移
  ...
  .bar-inner {
    ...
    .progress-btn-wrapper {
      ...
      .progress-btn {
        ...
        width: 0.32*100rem;
        height: 0.32*100rem;
        border: 0.06*100rem solid yellow;
        border-radius: 50%;
        background: red;
      }
    }
